...
## Data ----
county_df <-
    readRDS(here::here("data", "sensitivity_centers_of_pop_analytic_data_county_all.RDS"))
## Data ----
tract_df <- bind_rows(readRDS(here::here("data", "sensitivity_centers_of_pop_tract_all.RDS")),
                      readRDS(here::here("data", "sensitivity_centers_of_pop_tract_bupe.RDS")),
                      readRDS(here::here("data", "sensitivity_centers_of_pop_tract_otp.RDS"))) %>%
...
urc_summary <- super_tract %>%
    ...
    ungroup() %>%
    arrange(providers, metric, rank, urc_cat) %>%
    select(
        providers,
        urc_code,
        urc_cat,
...
write_csv(county_summary,
          "./output/sensitivity_centers_of_pop_raw_tableS01_county_summary.csv")
write_csv(urc_summary,
          "./output/sensitivity_centers_of_pop_raw_tableS02_urban_rural_summary_by_rank.csv")

print_urc <- urc_summary %>%
    filter(rank == 1, metric == "driving_time") %>%
    transmute(
        providers, 
        urc_cat,
        n_tracts,
...
write_csv(print_urc,
          "./output/sensitivity_centers_of_pop_table1_summary.csv")
