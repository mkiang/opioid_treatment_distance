...
county_df <- county_df %>%
    dplyr::group_by(providers,
                    ...
                    type,
                    metric) %>%
    dplyr::mutate(absolute_diff = median - min(median)) %>%
    dplyr::mutate(percent_diff = absolute_diff / min(median) * 100) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(
        trunc_abs_diff = dplyr::case_when(
        ...
        trunc_perc_diff = dplyr::case_when(
